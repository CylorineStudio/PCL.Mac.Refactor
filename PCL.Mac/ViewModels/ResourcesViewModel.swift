...
class ResourcesViewModel: ObservableObject {
        ...
        await MainActor.run {
            searchResults = response.hits.map { project in
                return .init(
                    ...
                    tags: project.categories.compactMap { tagMap[$0] },
                    supportDescription: generateSupportDescription(for: project),
                    downloads: formatDownloads(project.downloads),
                    lastUpdate: formatLastUpdate(project.lastUpdate)
                )
            }
        }
    }
    
    private func formatLastUpdate(_ lastUpdate: Date) -> String {
    ...
    private func generateSupportDescription(for project: ModrinthProject) -> String {
    ...
    private func generateGameVersionDescription(_ gameVersions: [String], latestVersion: String) -> String {
    ...
    private let tagMap: [String: String] = [
        "technology": "科技",
        "magic": "魔法",
        "adventure": "冒险",
        "utility": "实用",
